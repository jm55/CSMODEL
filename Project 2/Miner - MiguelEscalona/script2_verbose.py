...
class RuleMiner(object):
    ...
    def merge_itemsets(self, itemsets):
        ...
        if cur_num_items == 1:
            for i in range(len(itemsets)):
                ...
                #percent = i*j/(n2)*100
                #print('merge_itemset(if): {:.2f}%'.format(percent))
        else:
            for i in range(len(itemsets)):
                for j in range(i + 1, len(itemsets)):
                    combined_list = list(set(itemsets[i]).union(set(itemsets[j])))
                    combined_list.sort()
    ...
    def get_frequent_itemsets(self, data):
        ...
        while flag:
            new_itemsets = []
            print('get_frequent_itemsets: getting support @{:.0f}...'.format(len(itemsets)))
    ...
    def get_confidence(self, data, rule):
        ...
        if len(rule) == 2:
            ...
            x = x + rule[1]
            y = rule[0].copy()
            return self.get_support(data,x)/self.get_support(data,y)
        else:
            #3D list
            c=[]
            for r in rule:
                c.append(self.get_confidence(data,r))
            return c
    ...
    def get_association_rules(self, data):
        ...
        for rule in rules: #for every rule
            ...
            # this class.
            c = self.get_confidence(data,rule) #get confidence(list) for every rule
            for j in c:
                if j >= self.confidence_t: 
                    association_rules.append(rule)#append rule that meets requirement
