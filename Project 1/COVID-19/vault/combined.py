...
def aggregator(src_df,iso_code,continent,location,count): #Aggregates the given DataFrame to a grouped version
    ...
    for i in range(dateCount):
        ...
        if(observations == count): #Will run only if there is atleast 1 observation
...
covid_df = raw_df.copy(deep=True) #REFERENCED; ORIGINALLY ACTS AS A SOLE DATAFRAME
...
print("PLEASE USE covid_df AS THE LIVE DATAFRAME")
